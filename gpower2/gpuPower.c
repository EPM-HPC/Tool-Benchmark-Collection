...
\n\
    -o : specify which file to store the output info\n\n"
...
void *threadWork(void *arg)
{
    ...
    if (aFlag == 0)
        fp2 = fopen("out_power_measured_interval.csv", "w");
...
float correctedPowerSerial()
{
    ...
    for (i = 0; i < n_values; i++)
    {
        ...
        if (powers[i] / 1000.0 > 35 && begin_gpu == -1)
            begin_gpu = i;
        if (powers[i] / 1000.0 < 35 && begin_gpu != -1)
            end_gpu = i;
    ...
    if (wFlag == 0)
    {
        if (values_threshold > 0)
        {
            ...
            if (aFlag == 0)
                p_average_interval = acc3 / ((i_end - i_begin) * 1.0);
        }
        else
        {
    ...
    if (wFlag == 0){
        ...
        printf("\tAt current SM-frequency %d MHz:  Average Power: %.2f W;  Max Power: %.2f W;  Sampling Duration: %.2f ms;  GPU active duration: %.2f ms", oFreq, p_average / 1000.0, peak / 1000.0, (interval) / 1000, interval_GPU / 1000);
        avgPower= p_average / 1000.0 ;
        GPU_duration=interval_GPU / 1000;
    }
    if (wFlag == 0)
    {
        if (eFlag == 1){
    ...
    if (wFlag == 0)
        fprintf(fp, "Average Power [W],%.2f,Max Power [W],%.2f,Sampling Duration [ms],%f, Current frequency,(%d,%d) MHz,GPU active duration [ms],%f", p_average / 1000.0, peak / 1000.0, (interval) / 1000, mem_clock, core_clock, interval_GPU / 1000);
    if (wFlag == 0)
    {
        if (eFlag == 1)
            fprintf(fp, "\tEnergy: %.3f J\n", energy);
        else
            fprintf(fp, "\n");
    }
    ...
    fclose(fp2);
    snprintf(outputFile,sizeof(outputFile),"info-%s.txt",appname);	
    printf("%s\n",outputFile);
...
int ParseOptions(int argc, char **argv)
{
    ...
    int c;
    int err = 0;
    const char usage[] = "Usage: %s [-e] [-d device_id] [-t power_level] [-s sample_time_ms] [-o SM_freqency] [-f end_time_s] [-w] -a application_path\nType 'power_measure -h' for help.\n";
    int i = 0;
    int aux = 2;
    ...
    while ((c = getopt(argc, argv, "hcirewd:s:a:f:p:t:o:")) != -1)
    {
        i++;
        switch (c)
        {
...
int endProgram()
{
    ...
    {
        if (aFlag == 0)
            printf("File out_power_measured_interval.csv created with measured power samples in defined interval.\n");
